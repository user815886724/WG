/**
  1. [73] edit MOD_NAME = 'table' --> MOD_NAME = 'dsTablePro'
  2. [ajax] add 请求重新封装
  3. [236] add 默认样式 'line';
  4. [888] edit 没有记录不显示问题
  5. [902] edit layout: ['prev', 'page', 'next', 'skip', 'count', 'limit'] remove limit
  6. 排序问题 请求那里解决
 */
 
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util', 'request', 'utils'], function(exports){
  "use strict";
  ...
  //字符常量
  ,MOD_NAME = 'dsTable', ELEM = '.layui-table', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled', NONE = 'layui-none'
  ...
  Class.prototype.pullData = function(curr){
    ...
    if(options.url){ //Ajax请求
     ...
     params.pageParam = {
      limit: options.limit,
      pageIndex: curr,
      sortField: options.where.order ? layui.utils.formatHumpString(options.where.field, '_') : null,
      sortType: options.where.order || 'desc',
     };
      ...
      layui.request(options.url, {method: options.method || 'POST', data: data }).then(function(res) {
        // 记忆选中
        if(typeof options.checkedData === 'function'){
          var fieldName = options.checkedData().field;
          var fieldData = options.checkedData().data;
          $.each(res.data, function(i,d) {
            $.each(fieldData, function(i2,d2) {
        ...
        that.renderData(res, curr, res.pageParam.recordTotal), sort();
        options.time = (new Date().getTime() - that.startTime) + ' ms'; //耗时（接口请求+视图渲染）

        that.setColsWidth();
        typeof options.done === 'function' && options.done(res, curr, res.pageParam.recordTotal);
      });
      // ##### add.end 2019.03.21 #####
      /*
      $.ajax({
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    if(data.length === 0){
      ...
      return that.layMain.append('<div style="line-height: 26px;padding: 15px;text-align: center;color: #999;">没有记录</div>');
    }

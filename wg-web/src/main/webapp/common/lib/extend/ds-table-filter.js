...
layui.define(['dsTable', 'jquery', 'form', 'request'], function (exports) {

    var MOD_NAME = 'dsTableFilter',
		$ = layui.jquery,
		table = layui.dsTable,
		form = layui.form;
		...
		var main = function (){
			...
			layui.each(filters, function(i, filter){
				var filterField = filter.field,
					...
					filterKeyId = (typeof filter.key !== 'undefined' && typeof filter.key.id !== 'undefined') ? filter.key.id : 'id',
					filterKeyName = (typeof filter.key !== 'undefined' && typeof filter.key.name !== 'undefined') ? filter.key.name : 'name';
					
				//插入图标	
				var th = elem.next().find('.layui-table-header th[data-field="'+filterField+'"]');
				var icon = filterType == 'input' ? 'dsicon-search' : 'dsicon-filter';
				var filterIcon = $('<span class="layui-table-filter layui-inline"><i class="dsicon '+icon+'"></i></span>');
				...
				filterIcon.on("click", function(e) {
					...
					if(filterType == "checkbox"){
						...
						if(!filterUrl){
							layui.each(filterData, function(i, item){
								filterBox.find('ul').append('<li><input type="checkbox" name="'+filterName+'['+item[filterKeyId]+']" value="'+item[filterKeyId]+'" title="'+item[filterKeyName]+'" lay-skin="primary"></li>');
							})
						}
					}
					if(filterType == "radio"){
						...
						if(!filterUrl){
							...
							layui.each(filterData, function(i, item){
								filterBox.find('ul').append('<li><input type="radio" name="'+filterName+'" value="'+item[filterKeyId]+'" title="'+item[filterKeyName]+'"></li>');
							})
						}
					}
					...
					*/
					if((filterType == 'checkbox' || filterType == 'radio') && filterUrl){
						...
						layui.request(filterUrl, {method: filter.method || 'POST', data: filter.where || {} }).then(function(res) {
							filterBoxUl.empty();
							filterType == "radio" && filterBoxUl.append('<li><input type="radio" name="'+filterName+'" value="" title="All" checked></li>');
							layui.each(res.data, function(i, item){
								filterType == "checkbox" && filterBoxUl.append('<li><input type="checkbox" name="'+filterName+'['+item[filterKeyId]+']" value="'+item[filterKeyId]+'" title="'+item[filterKeyName]+'" lay-skin="primary"></li>');
								filterType == "radio" && filterBoxUl.append('<li><input type="radio" name="'+filterName+'" value="'+item[filterKeyId]+'" title="'+item[filterKeyName]+'"></li>');
							})
							form.render(null, 'table-filter-form');
							form.val("table-filter-form", tableFilter.toLayuiFrom(elemId, filterName, filterType));
						});
